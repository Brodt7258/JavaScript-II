...
let allCaps = [];
allCaps = runners.map(e => ({
  ...e,
  first_name: e.first_name.toUpperCase()
}));
console.log(allCaps); 
...
// Problem 1
// Find mean value of donation, list all runners who donated at or above the mean
const mean = runners.reduce((acc, cur) => acc + cur.donation, 0) / runners.length;
console.log('Mean donation = ', mean);
console.log(runners.filter(e => e.donation >= mean));

// Problem 2
// Get all runners whose last name begins with an S, or any letter after S
console.log(runners.filter(e => e.last_name.toUpperCase()[0] >= 'S'));

// Problem 3
// Generate an "official" "company" email for the runners.  Something that totally wouldn't go horribly wrong if done in real life.
console.log(runners.map(e => ({
  ...e,
  emailOfficial: `${e.first_name.substring(0, 5)}_${e.last_name.substring(0, 5)}@${e.company_name}.com`
})));